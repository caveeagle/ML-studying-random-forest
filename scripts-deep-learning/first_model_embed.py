...
df = df.drop(columns=["price", "cadastral_income"
                      ])  # cadastral_income - with postal_code
...
y_train, y_test = train_test_split(
    X_num, postal, X_other, y, test_size=0.2
)
...
unique_postals = np.sort(np.unique(X_train_postal))
...
X_test_postal_codes = np.array(
    [postal_to_index.get(x, -1) for x in X_test_postal])
